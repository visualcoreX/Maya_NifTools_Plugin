...
public:
	...
	// prepareDeform is called just before deform is called
	// to allow the deformer to detect if any cached setup
	// needs to be invalidated

	OPENMAYA_AVAILABLE(2025)
	OPENMAYA_ENUM(MPxChangeNotificationFlags,
		kNoChange			    = 0,
		kTopologyChange		    = (1<<0),
		kWeightsChange 		    = (1<<1)
	);
	...
	OPENMAYA_AVAILABLE(2025)
	virtual MStatus prepareDeform(MDataBlock& block, unsigned int multiIndex, unsigned int changeFlags);
	...
	OPENMAYA_AVAILABLE(2022)
    OPENMAYA_MODIFIED(2025, "multiIndex parameter added")
	// called from a potential GPU version of this node in order to access
	// specific (non-animated) setup data
	virtual MObject getFixedSetupData(const MString& name, unsigned int multiIndex);
